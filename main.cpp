...
private:
    ...
    char** allWords;
public:
    PlagiarismChecker() :uniqueWords(nullptr), uniqueWordcount(0), numAllwords(0), docFrequency(nullptr), allWords(nullptr), cosSimilarities(0) {}
...
int main()
{
    ...
    removeStopWords();
    //docs[0].displayDocWords();
    //docs[1].displayDocWords();
    char** uniqueWords;
    int uniqueCount;
    int** documentFrequency;
    generateFrequencies(uniqueWords, uniqueCount, documentFrequency);
    //cout <<endl<< uniqueCount;
	//cout << endl << dotProdcalc(documentFrequency[0], documentFrequency[1]);
	//cout << endl <<fixed<<setprecision(2)<< calcMagnitude(documentFrequency[0], documentFrequency[1]);
	double** similarities;
	calculateAllCosineSimilarities(similarities, documentFrequency);
	//cout << endl << similarityIn(2, 1);
	cout << docs[0].getText() << endl << docs[1].getText();
    return 0;
}
...
void calculateAllCosineSimilarities(double**& similarities, int** documentFrequency)
{
    ...
    similarities = new double* [size];

    for (int i = 0; i < size; i++)
        similarities[i] = new double[size];
